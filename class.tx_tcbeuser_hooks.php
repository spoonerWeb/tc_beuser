...
//		if(strpos($GLOBALS['MCONF']['name'], 'txtcbeuser', 1) ) {
...
//		// --
//		
//		$hasModAccess = $GLOBALS['BE_USER']->modAccess($GLOBALS['MCONF'], true);
//		$isInModule   = (bool) strpos($GLOBALS['MCONF']['name'], 'txtcbeuser', 1);
//		
//		if($hasModAccess && $isInModule) {
//			$GLOBALS['BE_USER']->user['admin'] = 1;
//			$access = 31;
//		}
	...
	function processDatamap_afterDatabaseOperations($status, $table, $id, $fieldArray, $tce){
		if ($table == 'be_groups'){
			if (!empty($tce->datamap[$table][$id]['members'])){
				...
				if(strstr($id,'NEW')){	//if it's a new record
